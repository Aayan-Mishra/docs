...
description: "Modern Agent API for Artemis RL with built-in RL Teacher capabilities"title: "Agents"title: "Agents"
...
The Artemis Agent API provides a unified interface for working with AI agents across different providers (OpenAI, Anthropic, Local models). This API supports multiple providers (OpenAI, Anthropic, Local models) and includes structured feedback systems for reinforcement learning training.
...
my_agent = agent().config(
    ...
    level="balanced",## Quick Start## Quick Start
...
response = await my_agent.act("Hello, world!")from artemis.agent import agentfrom artemis.agent import agent
...
from artemis.agent import agent    provider="anthropic",    provider="anthropic",



# Returns an AgentBuilder instance    model_id="claude-3-sonnet-20240229"    model_id="claude-3-sonnet-20240229"
...
### `.config(**kwargs)`await my_agent.initialize()await my_agent.initialize()



Configure the agent with various parameters.response = await my_agent.act("Hello, world!")response = await my_agent.act("Hello, world!")
...
agent_instance = agent().config(
    ...
    model_id="claude-3-sonnet-20240229",  # Specific model## Factory Pattern## Factory Pattern
    ...
    frequency_penalty=0.0,             # Repetition penalty### `agent()`### `agent()`
    ...
    # Local model specificThe main factory function for creating agent instances.The main factory function for creating agent instances.
...
- **`"lenient"`** (1-3): Very forgiving, encourages experimentation

- **`"balanced"`** (4-6): Standard evaluation, balanced feedbackbuilder = agent()builder = agent()

- **`"strict"`** (7-8): High standards, detailed critiques

- **`"harsh"`** (9-10): Expert-level expectations, rigorous evaluation``````

- **`"beginner"`**: Optimized for learning, patient feedback

- **`"intermediate"`**: Moderate expectations, constructive feedback

- **`"expert"`**: Professional-grade evaluation
...
```python### `.config(**kwargs)`### `.config(**kwargs)`
...
openai_agent = agent().config(
    ...
    api_key="your-api-key",            # Optional if set in envConfigure the agent with various parameters.Configure the agent with various parameters.
...
```pythonagent_instance = agent().config(agent_instance = agent().config(

claude_agent = agent().config(

    provider="anthropic",    level="strict",                    # Grading/behavior level    level="strict",                    # Grading/behavior level

    model_id="claude-3-sonnet-20240229",  # claude-3-sonnet, claude-3-haiku

    api_key="your-api-key",            # Optional if set in env    provider="anthropic",              # Provider: openai, anthropic, local    provider="anthropic",              # Provider: openai, anthropic, local
...
```python    max_tokens=1000,                   # Maximum response length    max_tokens=1000,                   # Maximum response length

local_agent = agent().config(

    provider="local",    top_p=0.9,                        # Nucleus sampling    top_p=0.9,                        # Nucleus sampling

    model=pytorch_model,               # Your PyTorch model

    tokenizer=tokenizer,               # HuggingFace tokenizer    frequency_penalty=0.0,             # Repetition penalty    frequency_penalty=0.0,             # Repetition penalty

    device="cuda:0",                   # Device placement

    generation_config={                # Generation parameters    presence_penalty=0.0,              # Presence penalty    presence_penalty=0.0,              # Presence penalty

        "max_new_tokens": 512,

        "temperature": 0.7,    timeout=30.0,                      # Request timeout    timeout=30.0,                      # Request timeout
...
## Core Methods    model=None,                        # PyTorch model instance    model=None,                        # PyTorch model instance



### `.initialize()`    tokenizer=None,                    # Tokenizer instance    tokenizer=None,                    # Tokenizer instance



Initialize the agent and establish connections.    device="auto"                      # Device placement    device="auto"                      # Device placement
...
Generate a response to a prompt.### Level System### Level System
...

{
    ...
    "metadata": {- **`"lenient"`** (1-3): Very forgiving, encourages experimentation- **`"lenient"`** (1-3): Very forgiving, encourages experimentation

        "model": "claude-3-sonnet-20240229",

        "provider": "anthropic",- **`"balanced"`** (4-6): Standard evaluation, balanced feedback- **`"balanced"`** (4-6): Standard evaluation, balanced feedback

        "timestamp": "2025-09-15T10:30:00Z",

        "tokens_used": 45- **`"strict"`** (7-8): High standards, detailed critiques- **`"strict"`** (7-8): High standards, detailed critiques
...
- `prompt` (str): The input prompt- **`"intermediate"`**: Moderate expectations, constructive feedback- **`"intermediate"`**: Moderate expectations, constructive feedback

- `system_prompt` (str, optional): System-level instructions

- `context` (List[Dict], optional): Conversation history- **`"expert"`**: Professional-grade evaluation- **`"expert"`**: Professional-grade evaluation
...
### `.grade(student_response, reference_answer=None, prompt=None, **kwargs)`
...
grading = await teacher.grade(```python```python

    student_response="def reverse_string(s): return s[::-1]",

    reference_answer="Standard slicing approach",openai_agent = agent().config(openai_agent = agent().config(
...
# Response format:    model_id="gpt-4",                  # gpt-4, gpt-3.5-turbo, etc.    model_id="gpt-4",                  # gpt-4, gpt-3.5-turbo, etc.

{

    "score": 85,    api_key="your-api-key",            # Optional if set in env    api_key="your-api-key",            # Optional if set in env

    "feedback": "Good implementation using slicing. Clean and efficient.",

    "breakdown": {    organization="your-org-id"         # Optional    organization="your-org-id"         # Optional
    ...
    "strengths": [
        ...
        "Correct algorithm",#### Anthropic#### Anthropic

        "Clean syntax",

        "Efficient approach"```python```python

    ],

    "weaknesses": [claude_agent = agent().config(claude_agent = agent().config(

        "Missing docstring",

        "No input validation"    provider="anthropic",    provider="anthropic",

    ],

    "suggestions": [    model_id="claude-3-sonnet-20240229",  # claude-3-sonnet, claude-3-haiku    model_id="claude-3-sonnet-20240229",  # claude-3-sonnet, claude-3-haiku

        "Add type hints",

        "Include error handling",    api_key="your-api-key",            # Optional if set in env    api_key="your-api-key",            # Optional if set in env
...
- **Readability**: Code clarity and structure#### Local Models#### Local Models

- **Style**: Following language conventions

- **Documentation**: Comments and docstrings```python```python



### `.reward(student_response, reference_answer=None, prompt=None, **kwargs)`local_agent = agent().config(local_agent = agent().config(



Generate reward signals for reinforcement learning.    provider="local",    provider="local",



```python    model=pytorch_model,               # Your PyTorch model    model=pytorch_model,               # Your PyTorch model

reward = await teacher.reward(

    student_response="def factorial(n): return 1 if n <= 1 else n * factorial(n-1)",    tokenizer=tokenizer,               # HuggingFace tokenizer    tokenizer=tokenizer,               # HuggingFace tokenizer
...
# Response format:    generation_config={                # Generation parameters    generation_config={                # Generation parameters

{

    "reward": 0.75,        "max_new_tokens": 512,        "max_new_tokens": 512,

    "confidence": 0.88,

    "reasoning": "Correct recursive implementation with base case",        "temperature": 0.7,        "temperature": 0.7,

    "components": {

        "correctness_reward": 0.9,        "do_sample": True        "do_sample": True
...
- **Negative rewards**: Errors, poor practices, inefficient code## Core Methods## Core Methods

- **Zero reward**: Neutral or baseline performance



### `.prefer(response_a, response_b, prompt=None, **kwargs)`
...
preference = await teacher.prefer(Initialize the agent and establish connections.Initialize the agent and establish connections.
...
# Response format:await agent_instance.initialize()await agent_instance.initialize()

{
    ...
    "comparison": {### `.act(prompt, **kwargs)`### `.act(prompt, **kwargs)`
...
- **"A"**: First response is preferredresponse = await agent.act("Write a Python function to sort a list")response = await agent.act("Write a Python function to sort a list")
...
## Advanced Features# Response format:# Response format:
...
response1 = await agent.act("What is Python?", context=context)        "model": "claude-3-sonnet-20240229",        "model": "claude-3-sonnet-20240229",

context.append({"role": "user", "content": "What is Python?"})

context.append({"role": "assistant", "content": response1["text"]})        "provider": "anthropic",        "provider": "anthropic",



response2 = await agent.act("Give me an example", context=context)        "timestamp": "2025-09-15T10:30:00Z",        "timestamp": "2025-09-15T10:30:00Z",
...
response = await agent.act(
    ...
    "Calculate 15 * 24",- `tools` (List[Dict], optional): Available tools/functions- `tools` (List[Dict], optional): Available tools/functions
...
# Grade multiple responses### `.grade(student_response, reference_answer=None, prompt=None, **kwargs)`### `.grade(student_response, reference_answer=None, prompt=None, **kwargs)`
...
grades = await asyncio.gather(*[Provide structured grading for a student response.Provide structured grading for a student response.
...
### Error Handlinggrading = await teacher.grade(grading = await teacher.grade(



```python    student_response="def reverse_string(s): return s[::-1]",    student_response="def reverse_string(s): return s[::-1]",

try:

    response = await agent.act("Complex prompt")    reference_answer="Standard slicing approach",    reference_answer="Standard slicing approach",

except AgentInitializationError:

    print("Agent not properly initialized")    prompt="Write a function to reverse a string"    prompt="Write a function to reverse a string"
    ...
    print(f"Unexpected error: {e}")# Response format:# Response format:
...
from artemis.rl_algorithms.grpo import GRPOTrainer, GRPOConfig    "breakdown": {    "breakdown": {



# Setup teacher and student        "correctness": 90,        "correctness": 90,

teacher = agent().config(level="strict", provider="anthropic")

student = agent().config(level="beginner", provider="local", model=llama_model)        "efficiency": 85,        "efficiency": 85,



await teacher.initialize()        "readability": 80,        "readability": 80,
...
training_data = []        "documentation": 60        "documentation": 60
...
for prompt in coding_problems:
    ...
    teacher_reward = await teacher.reward(student_response["text"], prompt=prompt)    "strengths": [    "strengths": [

    

    training_data.append({        "Correct algorithm",        "Correct algorithm",

        "prompt": prompt,

        "response": student_response["text"],        "Clean syntax",        "Clean syntax",

        "reward": teacher_reward["reward"],

        "grade": teacher_grade["score"]        "Efficient approach"        "Efficient approach"
...
gpro_config = GRPOConfig(learning_rate=1e-5, batch_size=4)    "weaknesses": [    "weaknesses": [
...
preference_pairs = create_preference_pairs(training_data)        "No input validation"        "No input validation"
...
## Best Practices    "suggestions": [    "suggestions": [



### 1. Level Selection        "Add type hints",        "Add type hints",

- Use **"strict"** for training high-quality models

- Use **"balanced"** for general evaluation        "Include error handling",        "Include error handling",
...
- **Local models**: Full control and privacy    "confidence": 0.92    "confidence": 0.92
...
- Use batch processing for multiple evaluations**Grading Components:****Grading Components:**

- Set appropriate `timeout` values

- Configure `max_retries` for reliability- **Correctness**: Algorithm accuracy and edge case handling- **Correctness**: Algorithm accuracy and edge case handling



### 5. Error Handling- **Efficiency**: Time and space complexity- **Efficiency**: Time and space complexity

- Always wrap API calls in try-catch blocks

- Handle rate limiting gracefully- **Readability**: Code clarity and structure- **Readability**: Code clarity and structure
...
config = AgentConfig(
    ...
    model_name="gpt-4"Generate reward signals for reinforcement learning.Generate reward signals for reinforcement learning.
...
```pythonreward = await teacher.reward(reward = await teacher.reward(
...
my_agent = agent().config(
    ...
    provider="openai",    prompt="Write a factorial function"    prompt="Write a factorial function"
...
1. **Factory pattern**: `agent()` instead of direct instantiation# Response format:# Response format:
...
5. **Provider abstraction**: Unified interface across providers    "reward": 0.75,    "reward": 0.75,



## Examples    "confidence": 0.88,    "confidence": 0.88,



### Basic Usage    "reasoning": "Correct recursive implementation with base case",    "reasoning": "Correct recursive implementation with base case",

```python

# Create and use an agent    "components": {    "components": {

chatbot = agent().config(level="balanced", provider="openai")

await chatbot.initialize()        "correctness_reward": 0.9,        "correctness_reward": 0.9,

response = await chatbot.act("Explain quantum computing")

print(response["text"])        "efficiency_reward": 0.6,        "efficiency_reward": 0.6,
...
reviewer = agent().config(level="strict", provider="anthropic")    "feedback": "Good recursive solution, consider iterative for efficiency"    "feedback": "Good recursive solution, consider iterative for efficiency"
...
```python- **Positive rewards**: Good performance, correct solutions- **Positive rewards**: Good performance, correct solutions

# Compare two implementations

judge = agent().config(level="balanced", provider="anthropic")- **Negative rewards**: Errors, poor practices, inefficient code- **Negative rewards**: Errors, poor practices, inefficient code
...
preference = await judge.prefer(solution_a, solution_b, prompt="Code quality")### `.prefer(response_a, response_b, prompt=None, **kwargs)`### `.prefer(response_a, response_b, prompt=None, **kwargs)`
...
tokenizer = AutoTokenizer.from_pretrained("microsoft/DialoGPT-medium")preference = await teacher.prefer(preference = await teacher.prefer(
...
local_agent = agent().config(    response_b="def add(x, y):\n    \"\"\"Add two numbers.\"\"\"\n    return x + y",    response_b="def add(x, y):\n    \"\"\"Add two numbers.\"\"\"\n    return x + y",

    provider="local",

    model=model,    prompt="Write an addition function"    prompt="Write an addition function"
...
response = await local_agent.act("Hello!")# Response format:# Response format:
...
- `agent()` → AgentBuilder    "confidence": 0.85,    "confidence": 0.85,



### Builder Methods    "margin": 0.65,    "margin": 0.65,
...
- `.initialize()` → None    "comparison": {    "comparison": {

- `.act(prompt, **kwargs)` → Dict

        "correctness": {"A": 1.0, "B": 1.0},        "correctness": {"A": 1.0, "B": 1.0},

### RL Teacher Methods

- `.grade(student_response, **kwargs)` → Dict        "documentation": {"A": 0.0, "B": 1.0},        "documentation": {"A": 0.0, "B": 1.0},

- `.reward(student_response, **kwargs)` → Dict

- `.prefer(response_a, response_b, **kwargs)` → Dict        "style": {"A": 0.7, "B": 0.9}        "style": {"A": 0.7, "B": 0.9}



### Configuration Parameters    },    },

- `level`: str - Behavior level (lenient/balanced/strict/harsh)

- `provider`: str - API provider (openai/anthropic/local)    "justification": "Both are correct, but B follows better practices"    "justification": "Both are correct, but B follows better practices"
...
All methods return structured dictionaries with consistent fields for easy parsing and integration with training pipelines.**Preference Values:****Preference Values:**
