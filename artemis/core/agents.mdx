...
- **`"balanced"`** (4-6): Standard evaluation, balanced feedback
...
### `.reward(student_response, reference_answer=None, prompt=None, **kwargs)`
...
response = await local_agent.act("Hello!")
```
...
### RL Teacher Methods
- `.grade(student_response, **kwargs)` → Dict
- `.reward(student_response, **kwargs)` → Dict
- `.prefer(response_a, response_b, **kwargs)` → Dict

### Configuration Parameters
- `level`: str - Behavior level (lenient/balanced/strict/harsh)
- `provider`: str - API provider (openai/anthropic/local)
...
### Response Formats
All methods return structured dictionaries with consistent fields for easy parsing and integration with training pipelines.
