...
graph TD
    ...
    B --> D[P(s_t+1, r_t+1 | s_t, a_t)]
    B --> E[Uncertainty Estimation σ_PM]
    
    C --> F[Base Action Distribution]
    E --> G[Uncertainty-Guided Exploration]
    F --> H[Final Action a_t]
    G --> H
    
    D --> I[Multi-Step Planning]
    I --> J[Risk-Adjusted Value Function]
    J --> K[Q_PRISM = E[R] - λ·Var[R]]
    
    K --> L[Optimal Action Selection]
    L --> H
```
